const dewall = () => {
  ...
  const modal = document.querySelector(
    ".modal.show.fade.ng-isolate-scope.ev.ev-open-modal-paywall-REQUIRE_LOGIN.in"
  );
  if (modal) {
    modal.remove();
  }
  ...
  const contentPageAdWrapDesktop = document.querySelectorAll(
    ".content-page-ad_wrap.desktop"
  );
  contentPageAdWrapDesktop.forEach((ad) => {
    ad.remove();
  });

  const contentPageAdWrapMobile = document.querySelectorAll(
    ".content-page-ad_wrap.mobile"
  );
  contentPageAdWrapMobile.forEach((mobileContentPageAdWrapper) => {
    mobileContentPageAdWrapper.remove();
  });

  const adMobileLeaderBoardWrap = document.querySelectorAll(
    ".ad-mobile-leaderboard_wrap"
  );
  ...
  const youMayAlsoLikeAdWrapper = document.querySelector(
    "#taboola-below-article-thumbnails1"
  );
